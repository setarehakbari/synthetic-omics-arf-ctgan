...
## ---------- Correlation-structure preservation (Reviewer 1, Comment 4) ----------
## Gene-gene Spearman correlation structure, compared via the Pearson
## correlation between vectorized upper-triangle entries ("concordance")
## and the mean absolute difference between them.
msg("Correlation-structure preservation (gene-gene Spearman correlations) ...")
real_gene_matrix <- as.matrix(real[, genes, drop = FALSE])
storage.mode(real_gene_matrix) <- "double"
cor_real <- cor(real_gene_matrix, method = "spearman")
upper_idx <- upper.tri(cor_real)
real_upper_vec <- cor_real[upper_idx]
rm(cor_real, real_gene_matrix)

correlation_preservation <- list()
for (g in gen_names) {
  msg("  Correlation structure: %s ...", g)
  syn_gene_matrix <- as.matrix(datasets[[g]][, genes, drop = FALSE])
  storage.mode(syn_gene_matrix) <- "double"
  cor_syn <- cor(syn_gene_matrix, method = "spearman")
  syn_upper_vec <- cor_syn[upper_idx]
  rm(cor_syn, syn_gene_matrix)
  correlation_preservation[[g]] <- tibble(
    generator = g,
    Correlation_structure_concordance = cor(real_upper_vec, syn_upper_vec, method = "pearson"),
    Mean_absolute_correlation_difference = mean(abs(real_upper_vec - syn_upper_vec))
  )
}
correlation_preservation_summary <- bind_rows(correlation_preservation)
write_csv(correlation_preservation_summary, file.path(tables_dir, "03b_correlation_structure_preservation.csv"))
print(correlation_preservation_summary)

## Merge into fidelity_summary so Table 1 carries both marginal and
## dependency-structure fidelity metrics.
fidelity_summary <- fidelity_summary |> left_join(correlation_preservation_summary, by = "generator")
...
for (g_idx in seq_along(gen_names)) {
  ...
  set.seed(SEED + 410000 + 1000 * g_idx); npdr_syn <- run_npdr_rank(syn, g)
  set.seed(SEED + 420000 + 1000 * g_idx); npdr_comb <- run_npdr_rank(comb, paste0("Real+", g))
...
master_summary <- tibble(generator = gen_names) |>
  ...
  left_join(fidelity_summary |> select(generator, KS_BH_FDR_pct, Wilcoxon_BH_FDR_pct,
                                        Correlation_structure_concordance, Mean_absolute_correlation_difference), by = "generator") |>
